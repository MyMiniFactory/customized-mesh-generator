...
def callback(
    ...
    if uploaded_successfully:
        try:
            all_good_so_far = generated_successfully and uploaded_successfully
            r = patch_customized_mesh(callback_url, success=all_good_so_far)
            if r.status_code == 200:
                patched_successfully = True
                logger.info('Mesh patched successfully as %s' % 'successful' if all_good_so_far else 'failed')
        except Exception as e:
            logger.error(e)
